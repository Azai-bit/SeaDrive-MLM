...
public:
  ...
  /** @param cur_pos 当前机器人/传感器位置（世界坐标），用于 frontier_range 过滤近端前沿；若无需过滤可传零向量 */
  void searchFrontiers(const Eigen::Vector3d& cur_pos = Eigen::Vector3d::Zero());
...
private:
  ...
  void expandFrontier(const Eigen::Vector3i& first, const Eigen::Vector3d& cur_pos /* , const int& depth, const int& parent_id */);
  ...
  double frontier_range_;  // 仅保留距离当前船位较远的前沿（滤除近端伪前沿），单位 m
  double min_frontier_z_;  // 前沿体素最低高度阈值（世界系），低于该值将被忽略
  bool enforce_fixed_view_z_;  // 是否将视点强制约束在固定高度平面
  double fixed_view_z_;        // 视点固定高度（世界系）

  // Utils
  shared_ptr<EDTEnvironment> edt_env_;
  unique_ptr<RayCaster> raycaster_;
  Eigen::Vector3d frontier_filter_center_;  // 过滤参考点：与 cur_pos 同步，用于 frontier_range 判断
};
