...
public:
  ...
  void inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
                       ...
                       const Eigen::Quaterniond& camera_q = Eigen::Quaterniond::Identity());
...
private:
  ...
  void integrateRay(const Eigen::Vector3d& end_pt, const Eigen::Vector3d& camera_pos, int end_occ,
                    Eigen::Vector3d& update_min, Eigen::Vector3d& update_max,
                    bool apply_min_z_filter);
...
struct MapParam {
  ...
  double min_valid_point_z_;
  bool use_active_sensor_rays_;
  int active_ray_vertical_lines_;
  int active_ray_horizontal_samples_;
  double active_ray_vertical_fov_min_deg_;
  double active_ray_vertical_fov_max_deg_;
  // Constant azimuth offset (rad) applied to active-ray model.
  // This is useful when the lidar azimuth convention differs by a fixed yaw.
  double active_ray_azimuth_offset_rad_;
