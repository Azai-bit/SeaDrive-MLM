...
// When true, ignore current planned trajectory and continuously publish a stop command
// using the latest /odom_world position.
...
// Yaw PD-like damping on reference generation (planning side)
static bool yaw_pd_enable_ = false;
static double yaw_pd_kp_ = 0.0;        // P term on yaw error magnitude
static double yaw_pd_kd_ = 0.3;        // D term on yaw error rate magnitude
static double yaw_pd_min_scale_ = 0.5; // don't shrink reference too much
...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  // New trajectory arrived, resume normal execution.
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  if (stop_requested_) {
    // Publish a stop cmd at current odom position.
    pos = Eigen::Vector3d(odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z);
    vel.setZero();
    acc.setZero();
    yaw = std::atan2(2.0 * (odom.pose.pose.orientation.w * odom.pose.pose.orientation.z +
  ...
  // Planning-side yaw damping: modify the reference yaw based on current odom
  // to suppress overshoot in the downstream yaw controller.
  if (yaw_pd_enable_) {
    const double yaw_meas = std::atan2(
        2.0 * (odom.pose.pose.orientation.w * odom.pose.pose.orientation.z +
               odom.pose.pose.orientation.x * odom.pose.pose.orientation.y),
        1.0 - 2.0 * (odom.pose.pose.orientation.y * odom.pose.pose.orientation.y +
                     odom.pose.pose.orientation.z * odom.pose.pose.orientation.z));

    // Planned yaw/yawdot already computed above.
    const double yaw_err = wrapAngleRad(yaw - yaw_meas);
    const double yaw_rate_meas = odom.twist.twist.angular.z;
    const double yaw_err_rate = yawdot - yaw_rate_meas;

    // Shrink the reference progression when the yaw error is large and/or changing fast.
    const double scale_denom =
        1.0 + yaw_pd_kp_ * std::fabs(yaw_err) + yaw_pd_kd_ * std::fabs(yaw_err_rate);
    const double scale = std::max(yaw_pd_min_scale_, 1.0 / scale_denom);

    yaw = yaw_meas + yaw_err * scale;
    yawdot = yawdot * scale;
  }

  // 原始 yaw / yawdot 逻辑保持不变，避免在下发层强行“折断”轨迹导致 180° 急转
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber stop_sub = node.subscribe("/planning/stop", 1, stopCallback);
  ...
  nh.param("yaw_pd_enable", yaw_pd_enable_, false);
  nh.param("yaw_pd_kp", yaw_pd_kp_, 0.0);
  nh.param("yaw_pd_kd", yaw_pd_kd_, 0.3);
  nh.param("yaw_pd_min_scale", yaw_pd_min_scale_, 0.5);
