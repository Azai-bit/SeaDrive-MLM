...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  nh.param("manager/planar_motion", planar_motion_, false);
  nh.param("manager/fixed_height", fixed_height_, 0.5);
...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d& start_pt,
  ...
  Eigen::Vector3d use_start_pt = start_pt;
  Eigen::Vector3d use_start_vel = start_vel;
  Eigen::Vector3d use_start_acc = start_acc;
  Eigen::Vector3d use_end_pt = end_pt;
  Eigen::Vector3d use_end_vel = end_vel;
  if (planar_motion_) {
    use_start_pt[2] = fixed_height_;
    use_end_pt[2] = fixed_height_;
    use_start_vel[2] = 0.0;
    use_start_acc[2] = 0.0;
    use_end_vel[2] = 0.0;
  }

  std::cout << "[Kino replan]: start: " << use_start_pt.transpose() << ", " << use_start_vel.transpose()
            << ", " << use_start_acc.transpose() << ", goal:" << use_end_pt.transpose() << ", "
            << use_end_vel.transpose() << endl;

  if ((use_start_pt - use_end_pt).norm() < 1e-2) {
    cout << "Close goal" << endl;
    return false;
  }

  Eigen::Vector3d init_pos = use_start_pt;
  Eigen::Vector3d init_vel = use_start_vel;
  Eigen::Vector3d init_acc = use_start_acc;
  ...
  int status = kino_path_finder_->search(use_start_pt, use_start_vel, use_start_acc, use_end_pt, use_end_vel, true);
  if (status == KinodynamicAstar::NO_PATH) {
    ROS_ERROR("search 1 fail");
    // Retry
    kino_path_finder_->reset();
    status = kino_path_finder_->search(use_start_pt, use_start_vel, use_start_acc, use_end_pt, use_end_vel, false);
    if (status == KinodynamicAstar::NO_PATH) {
...
void FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3d>& tour,
  ...
  for (int i = 0; i < pt_num; ++i) {
    Eigen::Vector3d p = tour[i];
    if (planar_motion_) p[2] = fixed_height_;
    pos.row(i) = p;
  }

  Eigen::Vector3d zero(0, 0, 0);
  Eigen::Vector3d use_vel = cur_vel;
  Eigen::Vector3d use_acc = cur_acc;
  if (planar_motion_) {
    use_vel[2] = 0.0;
    use_acc[2] = 0.0;
  }
  ...
  PolynomialTraj::waypointsTraj(pos, use_vel, zero, use_acc, zero, times, init_traj);
