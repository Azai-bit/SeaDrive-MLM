...
void BsplineOptimizer::setParam(ros::NodeHandle& nh) {
  ...
  nh.param("optimization/planar_motion", planar_motion_, false);
  nh.param("optimization/fixed_height", fixed_height_, 0.5);
...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  if (planar_motion_ && control_points_.cols() >= 3) {
    control_points_.col(2).setConstant(fixed_height_);
  }
  knot_span_ = dt;
...
void BsplineOptimizer::optimize() {
  ...
  for (int i = 0; i < point_num_; ++i)
    for (int j = 0; j < dim_; ++j) {
      double cij = control_points_(i, j);
      if (planar_motion_ && dim_ >= 3 && j == 2) cij = fixed_height_;
