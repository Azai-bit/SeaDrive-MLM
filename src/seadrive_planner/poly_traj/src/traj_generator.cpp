...
int main(int argc, char** argv) {
  ...
  // 固定 z，高度始终等于当前里程计 z（禁用 z 轴变化）
  const double z0 = odom.pose.pose.position.z;

  pos.row(0) = Eigen::Vector3d(odom.pose.pose.position.x, odom.pose.pose.position.y, z0);
  pos.row(1) = Eigen::Vector3d(-0.5, 0.5, z0);
  pos.row(2) = Eigen::Vector3d(0, 0, z0);
  pos.row(3) = Eigen::Vector3d(0.5, -0.5, z0);
  pos.row(4) = Eigen::Vector3d(1, 0, z0);
  pos.row(5) = Eigen::Vector3d(0.5, 0.5, z0);
  pos.row(6) = Eigen::Vector3d(0, 0, z0);
  pos.row(7) = Eigen::Vector3d(-0.5, -0.5, z0);
  pos.row(8) = Eigen::Vector3d(-1, 0, z0);
