...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/auto_trigger_enable", auto_trigger_enable_, true);
  nh.param("fsm/auto_trigger_delay", auto_trigger_delay_, 1.0);
  ...
  fd_->trigger_ = false;
  wait_trigger_enter_time_ = ros::Time(0);
  frontier_log_enabled_ = false;

  // Initialize frontier-time CSV logging immediately at startup.
  frontier_log_start_time_ = ros::Time::now();
  ...
  frontier_log_filename_ = log_dir + "/reserve_sensor_frontier_time_" + std::string(buf) + ".csv";

  std::ofstream fout(frontier_log_filename_, std::ios::out);
  if (fout.is_open()) {
    fout << "time,valid_frontier" << std::endl;
    fout << 0.0 << "," << 0 << std::endl;
    frontier_log_enabled_ = true;
    ROS_INFO("[FastExplorationFSM] frontier log file: %s", frontier_log_filename_.c_str());
  } else {
    ROS_WARN("[FastExplorationFSM] failed to open frontier log file: %s", frontier_log_filename_.c_str());
  }
  ...
  // 阻塞等待首帧里程计，避免 FSM 一直输出 "no odom."
  ROS_INFO("[FastExplorationFSM] Waiting for first odom on /odom_world ...");
  while (ros::ok() && !fd_->have_odom_) {
    nav_msgs::OdometryConstPtr msg =
        ros::topic::waitForMessage<nav_msgs::Odometry>("/odom_world", nh, ros::Duration(5.0));
    if (msg) {
      odometryCallback(msg);
      ROS_INFO("[FastExplorationFSM] Received first odom.");
      break;
    }
    ROS_WARN("[FastExplorationFSM] Still no odom on /odom_world, retrying...");
  }
}
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case WAIT_TRIGGER: {
      ...
      if (auto_trigger_enable_ && !fd_->trigger_ && wait_trigger_enter_time_.toSec() > 0.0) {
        const double wait_sec = (ros::Time::now() - wait_trigger_enter_time_).toSec();
        if (wait_sec >= auto_trigger_delay_) {
          fd_->trigger_ = true;
          FIRSTFLAG = true;
          trigger_time_ = ros::Time::now();
          ROS_INFO("[FastExplorationFSM] auto trigger fired after %.2f s", wait_sec);
          transitState(PLAN_TRAJ, "FSM(auto_trigger)");
        }
      }
      break;
    }
    ...
    case FINISH: {
        // In sparse-return scenes, frontiers may appear after a short delay.
        // Auto-resume planning when frontier_callback has discovered new frontiers.
        if (auto_trigger_enable_ && !expl_manager_->ed_->frontiers_.empty()) {
          ROS_INFO_THROTTLE(1.0, "[FastExplorationFSM] Resume from FINISH: frontier available.");
          transitState(PLAN_TRAJ, "FSM(finish_resume)");
          break;
        }

        if (FIRSTFLAG)
        {
          FIRSTFLAG = false;  // 这里我顺便帮你修正了变量拼写
          TimeSum = ros::Time::now() - trigger_time_;
        }
        ROS_INFO_THROTTLE(1.0, "finish exploration. TimeSum = %.2f s", TimeSum.toSec());
  break;
    }
    ...
    case PLAN_TRAJ: {
      ...
      if (res == SUCCEED) {
        ...
        // Exploration finished but the previous traj might still be executing.
        // Trigger an extra replan to shorten/abort the current trajectory quickly,
        // so the adapter can converge to a nearby (almost current) target and stop.
        replan_pub_.publish(std_msgs::Empty());
    ...
    case PUB_TRAJ: {
      ...
      if (dt > 0) {
        ...
        vis_thread.detach();
        //waitForUserInput("Breakpoint reached after viewpoint computation");
      }
      break;
    }
...
void FastExplorationFSM::frontierCallback(const ros::TimerEvent& e) {
  ...
  valid_frontier_num = ed->frontiers_.size();
    ...
    valid_frontier_num = expl_manager_->ed_->frontiers_.size();
  }

  if (frontier_log_enabled_) {
    const double t_now = (ros::Time::now() - frontier_log_start_time_).toSec();
    std::ofstream fout(frontier_log_filename_, std::ios::app);
    if (fout.is_open()) {
      fout << t_now << "," << valid_frontier_num << std::endl;
    }
  }
...
void FastExplorationFSM::transitState(EXPL_STATE new_state, string pos_call) {
  ...
  state_ = new_state;
  if (new_state == WAIT_TRIGGER) {
    wait_trigger_enter_time_ = ros::Time::now();
  }
