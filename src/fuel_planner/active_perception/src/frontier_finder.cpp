...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, ros::NodeHandle& nh) {
  ...
  nh.param("frontier/frontier_range", frontier_range_, 0.0);
  nh.param("frontier/min_frontier_z", min_frontier_z_, 0.0);
  nh.param("frontier/enforce_fixed_view_z", enforce_fixed_view_z_, true);
  nh.param("frontier/fixed_view_z", fixed_view_z_, 0.5);
...
void FrontierFinder::searchFrontiers(const Eigen::Vector3d& cur_pos) {
  ...
  frontier_filter_center_ = cur_pos;
  ...
  auto resetFlag = [&](list<Frontier>::iterator& iter, list<Frontier>& frontiers) {
    ...
    for (auto cell : iter->cells_) {
      ...
      // 防止越界：仅在索引有效时才重置 frontier_flag_
  ...
  for (int x = min_id(0); x <= max_id(0); ++x)
    for (int y = min_id(1); y <= max_id(1); ++y)
      for (int z = min_id(2); z <= max_id(2); ++z) {
        ...
        if (!inmap(cur)) continue;  // 先保证索引在地图范围内，避免对 frontier_flag_ 越界访问
...
void FrontierFinder::expandFrontier(
  const Eigen::Vector3i& first, const Eigen::Vector3d& cur_pos /* , const int& depth, const int& parent_id */) {
  ...
  while (!cell_queue.empty()) {
    ...
    for (auto nbr : nbrs) {
      ...
      // 新增：采样点与当前无人船距离判断
      if ((pos - cur_pos).norm() < 5.0) {
        continue;
      }

      edt_env_->sdf_map_->indexToPos(nbr, pos);
      if (pos[2] < min_frontier_z_) continue;  // Filter voxels below configured frontier height
      expanded.push_back(pos);
      cell_queue.push(nbr);
...
void FrontierFinder::updateFrontierCostMatrix() {
  // Debug logs below were causing excessive console output during exploration,
  // comment them out to keep runtime output concise.
...
void FrontierFinder::computeFrontiersToVisit() {
  ...
  for (auto& tmp_ftr : tmp_frontiers_) {
    // 若设置了 frontier_range_，先按当前机器人位置过滤掉离得太远的前沿簇，避免无意义的大范围跳跃
    if (frontier_range_ > 0.0) {
      const double dist_xy =
          (tmp_ftr.average_.head<2>() - frontier_filter_center_.head<2>()).norm();
      if (dist_xy > frontier_range_) {
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  // 若经过体素滤波后已经没有前沿体素，直接返回，不再尝试生成视点
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      if (enforce_fixed_view_z_) sample_pos[2] = fixed_view_z_;
