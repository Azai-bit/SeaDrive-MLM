...
public:
  ...
  // 标记某个节点（或一条路径上的所有节点）已被访问过，用于在后续规划中增加历史代价，避免反复访问同一 viewpoint
  static void addVisit(const ViewNode::Ptr& node);
  static void addVisits(const vector<ViewNode::Ptr>& nodes);
  ...
  static double w_hist_;  // 历史访问代价权重
  static shared_ptr<Astar> astar_;
  static shared_ptr<RayCaster> caster_;
  static shared_ptr<SDFMap> map_;

  // 记录每个节点被选入局部路径的次数，用于在 costTo 中增加历史代价
  static unordered_map<int, int> visit_counts_;
};
}
#endif